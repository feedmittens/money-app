...
- [ ] **AI finance assistant** — chat interface that answers questions about your finances ("how much did I spend on groceries last month?", "when will I run out of money at this rate?"). Would call the Anthropic API server-side (never expose the API key to the browser) with a structured context payload (balances, category totals, upcoming bills — NOT raw transaction history). Guardrail concerns to solve before building: (1) what data gets sent to Anthropic and how to minimize it; (2) user consent / disclosure that data leaves the server; (3) cost control (token limits per query); (4) whether to store chat history in the DB or keep it ephemeral. Don't build until these are thought through.

- [x] **Direct bank downloads via Plaid** — pull transactions automatically from financial institutions. Approach: Plaid Link (client-side OAuth flow) → exchange public token for access token → store access token server-side → `POST /api/plaid/sync` polls Plaid Transactions API and writes new transactions to the DB. Plaid has a free dev tier (up to 100 Items). Needs: `PLAID_CLIENT_ID` + `PLAID_SECRET` in config.env, new `plaid_items` table (user_id, access_token, cursor), `plaid` npm package on server, Plaid Link JS on client. Server runs as the only party that ever sees the access token — never expose it to the browser.
