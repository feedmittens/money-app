...
function parseQif(text) {
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith('!Account')) {
      flushTxn();
      currentAccount = { name: '', type: pendingType || 'checking', transactions: [] };
      pendingType = null;
    ...
    if (line.startsWith('!Type:')) {
      ...
      const mapped = QIF_ACCOUNT_TYPE[t] || 'checking';
      if (currentAccount) {
        currentAccount.type = mapped;
      } else {
        pendingType = mapped;
      }
      continue;
    ...
    if (!currentAccount) {
      currentAccount = { name: 'Imported Account', type: pendingType || 'checking', transactions: [] };
      pendingType = null;
      accounts.push(currentAccount);
    }
...
module.exports = { parseQifDate, parseAmount, parseQif, parseOfxDate, parseOfx, parseCsv, parseFile };
